...
function Results({ onRun }) {
    const resultJson = require("../../track/c10k/result.js")
    const { config } = useContext(GlobalContext)
    let invalidConfig = false
    const result = useMemo(() => {
        const a = resultJson[encode(JSON.stringify(config))];
        if (!a) {
            invalidConfig = true;
            return resultJson[encode(JSON.stringify(null))]
        }
        return a;
    }, [onRun])
        ...
        return (
            <>
                ...
                <div className='board'>
                    ...
                    <div className={((onRun == 0 || invalidConfig) && 'blur-md' || null) + ' board-area relative'}>
